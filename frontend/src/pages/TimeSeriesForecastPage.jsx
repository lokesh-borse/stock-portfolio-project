...
export default function TimeSeriesForecastPage() {
  ...
  async function onRunForecast() {
    ...
    setError('')
    try {
      const res = await fetchPortfolioTimeSeriesForecast(portfolioId, symbol, horizonDays)
      setResult(res)
    } catch (e) {
      setResult(null)
      setError(e?.response?.data?.detail || 'Failed to run time-series forecast.')
    } finally {
      setRunning(false)
    }
  }
  ...
  const chartData = useMemo(() => {
    ...
    return {
      ...
      datasets: [
        ...
        {
          label: `ARIMA Forecast (TS-${result.selected_horizon_days})`,
  ...
  return (
    <div className="mx-auto w-[96vw] max-w-[1700px] p-4 md:p-6">
      <div className="rounded-3xl border border-white/70 bg-gradient-to-r from-cyan-50 via-teal-50 to-emerald-50 p-6 shadow-lg shadow-teal-900/10 mb-6">
        ...
        <h1 className="text-3xl font-extrabold text-slate-900">Time Series Forecast (ARIMA)</h1>
      ...
      {result ? (
        <>
          ...
          <div className="rounded-2xl border border-white/80 bg-white/90 p-5 shadow-sm">
            <h2 className="text-2xl font-bold text-slate-900 mb-3">Selected Forecast Points (TS-{result.selected_horizon_days})</h2>
